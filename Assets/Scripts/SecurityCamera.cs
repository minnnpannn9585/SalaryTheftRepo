...
public class SecurityCamera : MonoBehaviour
{
    ...
    public Color slackingColor = Color.magenta; // 发现玩家摸鱼时的颜色
    ...
    public bool playerSlacking = false; // 是否检测到玩家在摸鱼
    ...
    void DetectPlayer()
    {
            ...
            if (collider.CompareTag(playerTag))
            {
                ...
                if (angleToPlayer <= detectionAngle / 2f)
                {
                    ...
                    if (characterStatus != null && characterStatus.isSlackingAtWork)
                    {
                        ...
                        if (enableDebug)
                        {
                            string penaltyMsg = penaltyApplied ? "惩罚已执行" : "惩罚冷却中";
                            Debug.Log($"[SecurityCamera] 🚨 检测到玩家在摸鱼！{penaltyMsg}");
                        }
                    }

                    if (enableDebug)
                    {
                        string status = playerSlacking ? "摸鱼中" : "正常状态";
                        Debug.Log($"[SecurityCamera] ✅ 玩家检测成功！状态: {status}");
                    }
                    break;
                }
                else
        ...
        {
            Debug.Log("[SecurityCamera] ⚠️ 没有检测到任何碰撞体，检查detectionRange设置");
        }
    ...
    void UpdateSpotlightColor()
    {
        if (spotlight != null)
        {
            if (playerSlacking)
            {
                spotlight.color = slackingColor; // 摸鱼时用特殊颜色
            }
            else if (playerDetected)
            {
                spotlight.color = alertColor; // 检测到玩家但没摸鱼用警报颜色
            }
            else
            {
                spotlight.color = normalColor; // 正常状态
            }
        }
    }
